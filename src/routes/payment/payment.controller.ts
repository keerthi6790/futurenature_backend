            ...
            try {
                ...
                if (fullOrder) {
                    const orderData = {
                        ...
                        customerName: `${(fullOrder.user as any)?.firstName || ""} ${(fullOrder.user as any)?.lastName || ""}`.trim() || "N/A",
                        ...
                        items: fullOrder.items.map((item: any) => `${item.product.product_name} (x${item.selected_quantity})`).join(", "),
                        address: fullOrder.address ? `${fullOrder.address.address1}, ${fullOrder.address.city}, ${fullOrder.address.state} - ${fullOrder.address.pincode}` : "N/A",
